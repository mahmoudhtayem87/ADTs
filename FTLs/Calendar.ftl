...
<#if entries?has_content>
    ...
    <#list entries as curEntry>
        ...
        <#assign title=assetObject.getTitle(locale,true) />
        <#assign description=assetObject.getDescription(locale,true)! "" />
        <#assign id=assetObject.getCalendarBookingId() />
        <#assign StartDate=assetObject.getStartTime()?number_to_datetime />
        <#assign EndDate=assetObject.getEndTime()?number_to_datetime />
        <#assign FormattedStartDate=StartDate?string[ "MMMM/d/yyyy"] />
        <#assign FormattedEndDate=EndDate?string[ "MMMM/d/yyyy"] />
        <script>
            var event = {
                id: '${id}',
                name: `${title}`,
        ...
        <div class="events">
            <div class="card">
                <div class="card-header " style="padding:0!important;margin:0">
                    <div class="row bg-custom-primary borders p-2">
                        <div class="text-white col-auto mr-auto header-title">
                            <h1 class="middle">Events List</h1>
                        </div>
                        <div class="col-auto bg-custom-primary" onclick="closeEvents()">
                            <i class=" text-white fas fa-4x fa-times"></i>
                        </div>
                    </div>
                </div>
                <div class="card-body borders">
                    <div class="events-contents" style="overflow: scroll;" id="htmlContent">
